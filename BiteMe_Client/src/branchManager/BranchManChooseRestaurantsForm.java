...
/**
 * This class is the functionality for the BranchManChooseRestaurantsWindow.
 */
public class BranchManChooseRestaurantsForm implements IScreenController {
	...
	/** The restaurants scroll pane - to show all relevant restaurants. */
	@FXML
	private ScrollPane restaurantsScrollPane;

	/** The restaurants V box - to show all relevant restaurants list. */
	@FXML
	 ...
	 * Logout of account.
	 *
	 * @param event - an ActionEvent from the BranchManChooseRestaurantsWindow
	 */
	@FXML
	...
	/**
	 * Builds the new restaurant manger user H box in the screen. 
	 */
		...
		if (userList != null) {
			for (User u : userList) {
				...
				userRow.setOnMouseClicked(eventMouseClicked -> {
					...
					userViewController.getScreenManager().loadScreen(MyScreenEnum.BRANCHMAN_RESTAURANTS, ClientUI.class,
							MyScreenEnum.BRANCHMAN_RESTAURANTS.getFXMLName(), userViewController);
					userViewController.getScreenManager().setScreen(MyScreenEnum.BRANCHMAN_RESTAURANTS);
				});
				...
				restaurantsVBox.getChildren().add(userRow);
